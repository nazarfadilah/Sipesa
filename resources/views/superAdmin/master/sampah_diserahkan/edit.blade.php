            ...
            <form action="{{ route('superadmin.master.sampah-diserahkan.update', $sampahDiserahkan->id) }}" method="POST" id="sampahDiserahkanForm">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('tgl_diserahkan') is-invalid @enderror" id="tgl_diserahkan" name="tgl_diserahkan" value="{{ old('tgl_diserahkan', \Carbon\Carbon::parse($sampahDiserahkan->tgl_diserahkan)->format('Y-m-d')) }}" required>
                        @error('tgl')
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_user') is-invalid @enderror" id="id_user" name="id_user" required>
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('id_user', $sampahDiserahkan->id_user) == $user->id ? 'selected' : '' }}>{{ $user->instansi->nama_instansi }} - {{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('id_user')
                        <div class="invalid-feedback">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_lokasi') is-invalid @enderror" id="id_lokasi" name="id_lokasi" required>
                            ...
                            @foreach($lokasiAsals as $lokasi)
                                <option value="{{ $lokasi->id_lokasi }}" {{ old('id_lokasi', $sampahDiserahkan->id_lokasi) == $lokasi->id_lokasi ? 'selected' : '' }}>{{ $lokasi->nama_lokasi }}</option>
                            @endforeach
                        </select>
                        @error('id_lokasi')
                        <div class="invalid-feedback">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('kategori_jenis') is-invalid @enderror" id="kategori_jenis" name="kategori_jenis" required>
                            ...
                            @foreach($kategoriJenises as $kategori)
                                <option value="{{ $kategori }}" {{ old('kategori_jenis', $sampahDiserahkan->jenis->kategori_jenis) == $kategori ? 'selected' : '' }}>
                                    {{ ucfirst($kategori) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_jenis') is-invalid @enderror" id="id_jenis" name="id_jenis" required>
                            ...
                            @foreach($jenises as $jen)
                                <option value="{{ $jen->id_jenis }}" data-kategori="{{ $jen->kategori_jenis }}" {{ old('id_jenis', $sampahDiserahkan->id_jenis) == $jen->id_jenis ? 'selected' : '' }}>{{ $jen->nama_jenis }}</option>
                            @endforeach
                        </select>
                        @error('id_jenis')
                        <div class="invalid-feedback">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('id_tujuan') is-invalid @enderror" id="id_tujuan" name="id_tujuan" required>
                            <option value="">Pilih Tujuan</option>
                            @foreach($tujuanSampahs as $tuj)
                                <option value="{{ $tuj->id_tujuan }}" {{ old('id_tujuan', $sampahDiserahkan->id_tujuan) == $tuj->id_tujuan ? 'selected' : '' }}>{{ $tuj->nama_tujuan }}</option>
                            @endforeach
                        </select>
                        @error('id_tujuan')
                        <div class="invalid-feedback">
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control @error('alasan_edit') is-invalid @enderror" 
                                  id="alasan_edit" 
                                  name="alasan_edit" 
                                  rows="3" 
                                  maxlength="500"
                                  placeholder="Jelaskan alasan mengapa data ini diedit"
                                  required>{{ old('alasan_edit', $sampahDiserahkan->alasan_edit) }}</textarea>
                ...
                <div class="d-flex">
                    <a href="{{ route('superadmin.master.sampah-diserahkan') }}" class="btn btn-secondary me-2">
...
<script>
    $(document).ready(function() {
        ...
        $('#sampahDiserahkanForm').on('submit', function(e) {
